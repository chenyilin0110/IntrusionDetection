...
# build random forset
forest = ensemble.RandomForestClassifier(n_estimators=int(estimators))

# training
forestFit = forest.fit(train_resultNormalize, train_noStringTemp_Y)

# testing
y_test_prediction = forest.predict(test_resultNormalize)

accuracyfunction.accuracy(test_noStringTemp_Y, y_test_prediction)