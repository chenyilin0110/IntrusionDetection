import numpy as np
f = open('src/Plot/ICS/2class/f1-score.txt', "a")
pre = np.loadtxt('src/Plot/ICS/2class/ICS_2class_precision_Avg' + '.txt',dtype=np.str)
re = np.loadtxt('src/Plot/ICS/2class/ICS_2class_recall_Avg' + '.txt', dtype=np.str)
count = 0.0
f.write('\"' + "DS1" + '\" ')
for i in range(1,11):
...
f = open('src/Plot/ICS/3class/f1-score.txt', "a")
pre = np.loadtxt('src/Plot/ICS/3class/ICS_3class_precision_Avg' + '.txt',dtype=np.str)
re = np.loadtxt('src/Plot/ICS/3class/ICS_3class_recall_Avg' + '.txt', dtype=np.str)
count = 0.0
f.write('\"' + "DS1" + '\" ')
for i in range(1,11):
    count = (2 * float(pre[0][i]) * float(re[0][i]) / (float(pre[0][i]) + float(re[0][i])))
