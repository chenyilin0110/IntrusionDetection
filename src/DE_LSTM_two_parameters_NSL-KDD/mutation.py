...
def mutation(populationData, F, Ud, Ld, lrUd, lrLd):    
    ...
    for eachpopulation_colum in range(np.size(populationData, 0)):
        ...
        for eachpopulation_row in range(np.size(populationData,1)):
            ...
            if eachpopulation_row == 0: # first bit(neurons)
                if populationData[eachpopulation_colum][eachpopulation_row] >= Ud:
                    populationData[eachpopulation_colum][eachpopulation_row] = int(Ud - 1)
                elif populationData[eachpopulation_colum][eachpopulation_row] <= Ld:
                    populationData[eachpopulation_colum][eachpopulation_row] = int(Ld + 1)
            else:
                if populationData[eachpopulation_colum][eachpopulation_row] >= lrUd:
                    populationData[eachpopulation_colum][eachpopulation_row] = lrUd
                elif populationData[eachpopulation_colum][eachpopulation_row] <= lrLd:
                    populationData[eachpopulation_colum][eachpopulation_row] = lrLd
