...
def update(best, bestModel, eachiteration, bestSolution, eachIterationLocalBest, population, countCrossoverLossValue, countOriginalLossValue, crossoverModel, originalModel, selectionData, crossoverData, name):
    if eachiteration == 0:
    ...
    for eachPopulation in range(np.size(countCrossoverLossValue, 0)):
        for i in range(np.size(countCrossoverLossValue, 1)):
            if best >= countCrossoverLossValue[eachPopulation][i]:
                best = countCrossoverLossValue[eachPopulation][i]
                bestModel = crossoverModel[eachPopulation][i]
                for j in range(len(bestSolution)):
                    bestSolution[j] = crossoverData[eachPopulation][i][j]
    
    for i in range(np.size(countOriginalLossValue, 0)):
        if best >= countOriginalLossValue[i]:
            best = countOriginalLossValue[i]
