...
for eachiteration in range(int(iteration)):
    ...
    # Crossover
    crossoverData = crossover(populationDataOriginal, mutationData, float(CR1), float(CR2), float(CR3), Ud, Ld, lrUd, lrLd)
    
    # Selection
    countOriginalLossValue = np.zeros(int(population))
    originalModel = []
    countCrossoverLossValue = np.zeros((int(population), 5))
    crossoverModel = []
    ...
    # Update
    best, bestModel = update(best, bestModel, eachiteration, bestSolution, eachIterationLocalBest, int(population), countCrossoverLossValue, countOriginalLossValue, crossoverModel, originalModel, selectionData, crossoverData, name)
