...
for eachiteration in range(int(iteration)):
    ...
    if name == '0':
        if os.path.exists('src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + '_bs' + batchSize + '.pkl'):
            os.remove('src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + '_bs' + batchSize + '.pkl')
        torch.save(bestModel, 'src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + '_bs' + batchSize + '.pkl')
    else:
        if os.path.exists('src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + name + '_bs' + batchSize + '.pkl'):
            os.remove('src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + name + '_bs' + batchSize + '.pkl')
        torch.save(bestModel, 'src/DE_LSTM_NSL-KDD/result/DE_LSTM_' + outputLayer + name + '_bs' + batchSize + '.pkl')
