...
def testing(outputLayer, train_noStringTemp_X, x_test_tensor, y_test_tensor, cuda, name):
    if name == '0':
        lstm = torch.load('src/DE_LSTM_NSL-KDD_one_parameter/result/DE_LSTM_' + outputLayer + '.pkl')
    else:
        lstm = torch.load('src/DE_LSTM_NSL-KDD_one_parameter/result/DE_LSTM_' + outputLayer + name + '.pkl')
